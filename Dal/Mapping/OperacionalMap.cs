...
namespace Dal.Mapping {
  internal class OperacionalMap : EntityTypeConfiguration<Operacional> {
    public OperacionalMap() {
      ...
      this.Property(t => t.ViagensDU).HasColumnName("ViagensDU");
      this.Property(t => t.PercursoDU).HasColumnName("PercursoDU").HasPrecision(29, 3);
      ...
      this.HasRequired(p => p.Linha)
          .WithMany(f => f.Operacionais).HasForeignKey(k => k.LinhaId).WillCascadeOnDelete(false);

      this.HasOptional(t => t.Atendimento)
          .WithMany(f => f.Operacionais).HasForeignKey(k => k.AtendimentoId).WillCascadeOnDelete(false);
    }
  }
}
