  ...
  internal class LinhaMap : EntityTypeConfiguration<Linha> {
    public LinhaMap() {
      ...
      this.HasRequired(t => t.Empresa)
          .WithMany(t => t.Linhas).HasForeignKey(d => d.EmpresaId).WillCascadeOnDelete(false);

      this.HasRequired(t => t.EDominio)
          .WithMany(t => t.Linhas).HasForeignKey(d => d.DominioId).WillCascadeOnDelete(false);

      this.HasRequired(t => t.Operacao)
          .WithMany(t => t.Linhas).HasForeignKey(d => d.OperacaoId).WillCascadeOnDelete(false);

      this.HasRequired(t => t.CLinha)
          .WithMany(t => t.Linhas).HasForeignKey(d => d.Classificacao).WillCascadeOnDelete(false);
    }
  }
}
